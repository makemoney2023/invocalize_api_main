...
const checkDuplicateUsernameOrEmail = async (req, res, next) => {
	...
	if (!req.body.email || !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,5}$/i.test(req.body.email)) {
	...
	const decryptedPassword = CryptoHelper.decrypt(req.body.password);
	
	if (!req.body.password || decryptedPassword.length < 6) {
		return res.status(400).json({
